...
switch($act){
	case "capnhat":
		get_gioithieu();
		$template = "about_list/item_add";
		break;
	case "man_list":
	    get_lists();
	    $template = "about_list/lists";
	    break;
    case "add":
        $template = "about_list/item_add";
        break;
...
function delete(){
    ...
    if(isset($_GET['id'])){
        ...
        $d->setWhere('id', $id);
        $d->select();
        if($d->num_rows()==0) transfer("Dữ liệu không có thực", "index.php?com=about_list&act=man_list");
        $row = $d->fetch_array();
        delete_file(_upload_banner.$row['photo']);
        delete_file(_upload_banner.$row['thumb']);
        if($d->delete())
            redirect("index.php?com=about_list&act=man_list");
        else
            transfer("Xóa dữ liệu bị lỗi", "index.php?com=about_list&act=man_list");
    }else transfer("Không nhận được dữ liệu", "index.php?com=about_list&act=man_list");
}
function get_lists(){
    ...
    if($_REQUEST['hienthi']!='')
    {
        ...
        $sql_sp = "SELECT id,hienthi FROM #_gioithieu_list where id='".$id_up."'";
        $d->query($sql_sp);
        $cats= $d->result_array();
        $hienthi=$cats[0]['hienthi'];
        if($hienthi==0)
        {
        	$sqlUPDATE_ORDER = "UPDATE #_gioithieu_list SET hienthi =1 WHERE  id = ".$id_up."";
        	$d->query($sqlUPDATE_ORDER);
        }
        else
    	{
        	$sqlUPDATE_ORDER = "UPDATE #_gioithieu_list SET hienthi =0  WHERE  id = ".$id_up."";
        	$d->query($sqlUPDATE_ORDER);
    	}
	}

	$sql = "select * from #_gioithieu_list";
...
function get_gioithieu(){
	...
	$sql = "select * from #_gioithieu_list where id='".$id."'";
	$d->query($sql);
...
function save_gioithieu(){
	...
	if($id){
    	...
    	$d->setWhere('id', $id);
    	if(!$d->update($data))
    		transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=about_list&act=man_list");
    	redirect("index.php?com=about_list&act=man_list");
	}else{
	    if($photo = upload_image("file", 'jpg|png|gif|JPG|jpeg|JPEG',_upload_gioithieu,$file_name)){
	        $data['photo'] = $photo;

...
<table class="blue_table">
	...
	{
		$image = '';
		if (file_exists('../upload/tinloai1_1/'.$items[$i]['thumb'])&&!empty($items[$i]['thumb'])) 
		{
			$image = '../upload/tinloai1_1/'.$items[$i]['thumb'];
		}
		else
		{
			$image = '../upload/empty.png';
		}
	?>
	<tr>
     ...
     <a href="index.php?com=tinloai1_1&act=edit&id_list=<?=$items[$i]['id_list']?>&id_cat=<?=$items[$i]['id_cat']?>&id=<?=$items[$i]['id']?><?php if($_REQUEST['curPage']!='') echo'&curPage='. $_REQUEST['curPage'];?>" style="text-decoration:none;">
	  <img src="<?=$image?>" width="200"/>
        ...
        <a href="index.php?com=tinloai1_1&act=man&hienthi=<?=$items[$i]['id']?><?php if($_REQUEST['id_danhmuc']!='') echo'&id_danhmuc='. $_REQUEST['id_danhmuc'];?><?php if($_REQUEST['id_list']!='') echo'&id_list='. $_REQUEST['id_list'];?><?php if($_REQUEST['curPage']!='') echo'&curPage='. $_REQUEST['curPage'];?>"><img src="media/images/active_1.png" border="0" /></a>
		<?php 
		}
		if($items[$i]['hienthi']==0)
		{
		?>
         <a href="index.php?com=tinloai1_1&act=man&hienthi=<?=$items[$i]['id']?><?php if($_REQUEST['id_danhmuc']!='') echo'&id_danhmuc='. $_REQUEST['id_danhmuc'];?><?php if($_REQUEST['id_list']!='') echo'&id_list='. $_REQUEST['id_list'];?><?php if($_REQUEST['curPage']!='') echo'&curPage='. $_REQUEST['curPage'];?>"><img src="media/images/active_0.png"  border="0"/></a>
         <?php
		 }?>        </td>

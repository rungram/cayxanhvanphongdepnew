...
<?php 
			...
			$sql_tungdanhmuc="select * from #_product where hienthi =1 and ( hienngonngu =3 or hienngonngu =1) and id_list='$id' ".$order;
			$d->query($sql_tungdanhmuc);	
			$result_spnam=$d->result_array();	
			
			$d->reset();
			$sql_laylist="select * from #_product_list where hienthi =1 and ( hienngonngu =3 or hienngonngu =1) and id='$id'";
			$d->query($sql_laylist);	
...
<?php 
			...
			$d->reset();
			$order = isset($_GET['order']) ? 'order by gia '.$_GET['order'] : 'order by stt asc';
			$sql_tungdanhmuc="select * from #_product where hienthi =1 and ( hienngonngu =3 or hienngonngu =2) and id_list='$id' ".$order;
			$d->query($sql_tungdanhmuc);	
			$result_spnam=$d->result_array();	
			$d->reset();
			$sql_laylist="select * from #_product_list where hienthi =1 and ( hienngonngu =3 or hienngonngu =2) and id='$id'";
			$d->query($sql_laylist);	
			$result_laylist=$d->fetch_array();	
            ...
            <div id="pathway" class="clearfix">
                <div class="container">
                    <div id="breadcrumbs" itemscope="" itemtype="http://schema.org/BreadcrumbList">
                        ...
                        <span class="hidden-xs">→</span>
                        <span itemprop="itemListElement" itemscope="" itemtype="http://schema.org/ListItem">
                            <a itemprop="item"><span itemprop="name"><?=$result_laylist["ten_en"]?></span></a>
                            <meta itemprop="position" content="2">
                        </span>
                    </div><!-- #breadcrumbs -->
                </div>
                ...
                <div id="main">
                    <div id="main-content">
                        <div class="widget_product ">
                          <!-- banner image -->
                          <div class="box-heading">
                            <h4 class="title"><a  title="<?=$result_laylist["ten_en"]?>"><?=$result_laylist["ten_en"]?></a></h4>
                          </div>
                          <div class="box-content">
                          <?php
                        	for ($i=0;$i<count($result_spnam);$i++)
                        	...
                        	?>
                              <div class="col-lg-3 col-md-3 col-sm-3 col-xs-6 product-block"> <a href="chi-tiet-san-pham/<?=$result_spnam[$i]['tenkhongdau']?>-<?=$result_spnam[$i]['id']?>.html" title="<?=$result_spnam[$i]["ten_en"]?>"> <img src="upload/sanpham/<?php if($result_spnam[$i]["tc_big"]==1) echo $result_spnam[$i]["photo"]; else echo $result_spnam[$i]["photo"] ?>" class="img-responsive wp-post-image" alt="<?=$result_spnam[$i]["ten_en"]?>" itemprop="image"> </a>
                                  ...
                                  <span class="price"><span class="amount"><?php echo $gia;?></span></span>
                                  <button type="submit" onclick="addtocart(<?=$result_spnam[$i]['id']?>,1)" class="single_add_to_cart_button button alt btn btn-link">Add to cart</button>
                              </div>
                          <?php
                        	} 
                        	?>

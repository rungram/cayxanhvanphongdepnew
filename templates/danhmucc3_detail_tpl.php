...
<?php 
	...
	$d->reset();
	$sql_tungdanhmuc="select * from #_product where hienthi =1 and ( hienngonngu =3 or hienngonngu =1) and id_item='$id'  order by stt asc ";
	//die($sql_tungdanhmuc);
	$d->query($sql_tungdanhmuc);	
	$result_spnam=$d->result_array();	
	
	$d->reset();
	$sql_item="select * from #_product_item where hienthi =1 and ( hienngonngu =3 or hienngonngu =1) and id='$id'";
	$d->query($sql_item);	
	$result_item=$d->fetch_array();	
	
	$d->reset();
	$sql_laylist="select * from #_product_list where hienthi =1 and ( hienngonngu =3 or hienngonngu =1) and id='".$result_item['id_list']."'";
	$d->query($sql_laylist);	
	$result_laylist=$d->fetch_array();	
	
	$d->reset();
	$sql_laycat="select * from #_product_cat where hienthi =1 and ( hienngonngu =3 or hienngonngu =1) and id='".$result_item['id_cat']."'";
	$d->query($sql_laycat);	
...
<?php 
    $id =  addslashes($_GET['id']);
	$d->reset();
	$sql_tungdanhmuc="select * from #_product where hienthi =1 and ( hienngonngu =3 or hienngonngu =2) and id_item='$id'  order by stt asc ";
	//die($sql_tungdanhmuc);
	$d->query($sql_tungdanhmuc);	
	$result_spnam=$d->result_array();	
	
	$d->reset();
	$sql_item="select * from #_product_item where hienthi =1 and ( hienngonngu =3 or hienngonngu =2) and id='$id'";
	$d->query($sql_item);	
	$result_item=$d->fetch_array();	
	
	$d->reset();
	$sql_laylist="select * from #_product_list where hienthi =1 and ( hienngonngu =3 or hienngonngu =2) and id='".$result_item['id_list']."'";
	$d->query($sql_laylist);	
	$result_laylist=$d->fetch_array();	
	
	$d->reset();
	$sql_laycat="select * from #_product_cat where hienthi =1 and ( hienngonngu =3 or hienngonngu =2) and id='".$result_item['id_cat']."'";
	$d->query($sql_laycat);	
	$result_cat=$d->fetch_array();	
        ...
        <div id="pathway" class="clearfix">
            <div class="container">
                <div id="breadcrumbs" itemscope="" itemtype="http://schema.org/BreadcrumbList">
                    ...
                    <span class="hidden-xs">→</span>
                    <span itemprop="itemListElement" itemscope="" itemtype="http://schema.org/ListItem">
                        <a itemprop="item"><span itemprop="name"><?=$result_item["ten_en"]?></span></a>
                        <meta itemprop="position" content="2">
                    </span>
                </div><!-- #breadcrumbs -->
            </div>
            ...
            <div id="main">
                <div id="main-content">
                    <div class="widget_product ">
                      <!-- banner image -->
                      <div class="box-heading">
                        <h4 class="title"><a href="cay-cong-trinh/index.html" title="Cây công trình"><?=$result_laylist["ten_en"]?></a></h4>
                      </div>
                      <div class="box-content">
                      <?php
                    	for ($i=0;$i<count($result_spnam);$i++)
                    	{ 
                    	    $gia = (!empty($result_spnam[$i]['gia']))?number_format ($result_spnam[$i]['gia'],0,",",".")."&nbsp;₫":'Call for price';
                    	    
                    	?>
                          <div class="col-lg-3 col-md-3 col-sm-3 col-xs-6 product-block"> <a href="chi-tiet-san-pham/<?=$result_spnam[$i]['tenkhongdau']?>-<?=$result_spnam[$i]['id']?>.html" title="<?=$result_spnam[$i]["ten_en"]?>"> <img src="upload/sanpham/<?php if($result_spnam[$i]["tc_big"]==1) echo $result_spnam[$i]["photo"]; else echo $result_spnam[$i]["photo"] ?>" class="img-responsive wp-post-image" alt="<?=$result_spnam[$i]["ten_en"]?>" itemprop="image"> </a>

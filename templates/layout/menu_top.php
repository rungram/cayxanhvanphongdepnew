...
<?php
	$d->reset();
	$sql_list ="select *	from #_product_list where hienthi=1 and ( hienngonngu =3 or hienngonngu =1) order by stt asc limit 0,8";
	$d->query($sql_list);
	$list =$d->result_array();
	
	
	$d->reset();
	$sql_gioithieu="select * from #_gioithieu_list  where hienthi=1 and ( hienngonngu =3 or hienngonngu =1) order by stt desc";
...
<ul id="menu-main-menu" class="nav navbar-nav">
        ...
        <ul role="menu" class=" dropdown-menu">
            <?php 
                for($i=0,$count_l=count($result_gioithieu);$i<$count_l;$i++){
                    $d->reset();
                    $sql_cat ="select *  from #_gioithieu where id_list='".$result_gioithieu[$i]["id"]."' and ( hienngonngu =3 or hienngonngu =1) order by stt asc";
                    $d->query($sql_cat);
                    $cat =$d->result_array();
    ...
    <?php for($i=0,$count_l=count($list);$i<$count_l;$i++){
            $d->reset();
            $sql_cat ="select *  from #_product_cat where id_list='".$list[$i]["id"]."' and ( hienngonngu =3 or hienngonngu =1) order by stt asc";
            $d->query($sql_cat);
            $cat =$d->result_array();
            $child = 'class="has-submenu" id="sm-15003598033380005-5" aria-haspopup="true" aria-controls="sm-15003598033380005-6" aria-expanded="false"';
    ...
    <li id="menu-item-2747" class="<?=$dropdown?>">
        ...
        <ul role="menu" class=" dropdown-menu">
            <?php for($j=0,$count_c=count($cat);$j<$count_c;$j++){ 
                $d->reset();
                $sql_item ="select *  from #_product_item where id_list='".$list[$i]["id"]."' and  id_cat='".$cat[$j]["id"]."' and ( hienngonngu =3 or hienngonngu =1) order by stt asc";
                $d->query($sql_item);
                $item =$d->result_array();
    ...
    <?php 
    $d->reset();
    $sql_list ="select *	from #_tinloai1_1_list where hienthi=1 and ( hienngonngu =3 or hienngonngu =1) order by stt asc limit 0,8";
    $d->query($sql_list);
    $list =$d->result_array();
    
    for($i=0,$count_l=count($list);$i<$count_l;$i++){
            $d->reset();
            $sql_cat ="select *  from #_tinloai1_1_cat where id_list='".$list[$i]["id"]."' and ( hienngonngu =3 or hienngonngu =1) order by stt asc";
            $d->query($sql_cat);
            $cat =$d->result_array();
            $child = 'class="has-submenu" id="sm-15003598033380005-5" aria-haspopup="true" aria-controls="sm-15003598033380005-6" aria-expanded="false"';
...
<?php
	$d->reset();
	$sql_list ="select *	from #_product_list where hienthi=1 and ( hienngonngu =3 or hienngonngu = 2) order by stt asc limit 0,8";
	$d->query($sql_list);
	$list =$d->result_array();
	
	
	$d->reset();
	$sql_gioithieu="select * from #_gioithieu_list  where hienthi=1 and ( hienngonngu =3 or hienngonngu = 2) order by stt desc";
	$d->query($sql_gioithieu);
	$result_gioithieu=$d->result_array();
	//$id_gioithieu = $result_gioithieu["id"];
?>
...
<ul id="menu-main-menu" class="nav navbar-nav">
    ...
    <li id="menu-item-5" class="menu-item menu-item-type-taxonomy menu-item-object-category menu-item-has-children menu-item-5 dropdown">
        ...
        <ul role="menu" class=" dropdown-menu">
            <?php 
                for($i=0,$count_l=count($result_gioithieu);$i<$count_l;$i++){
                    $d->reset();
                    $sql_cat ="select *  from #_gioithieu where id_list='".$result_gioithieu[$i]["id"]."' and ( hienngonngu =3 or hienngonngu = 2) order by stt asc";
                    $d->query($sql_cat);
                    $cat =$d->result_array();
                    $child = 'class="has-submenu" id="sm-15003598033380005-5" aria-haspopup="true" aria-controls="sm-15003598033380005-6" aria-expanded="false"';
                    $href = 'gioi-thieu-list/'.$result_gioithieu[$i]["tenkhongdau"].'-'.$result_gioithieu[$i]["id"].'.html';
            ...
            ?>
            <li id="menu-item-12" class="<?=$dropdown?>">
                <a title="<?=$result_gioithieu[$i]["ten_en"]?>" href="<?=$href?>"><?=$result_gioithieu[$i]["ten_en"]?></a>
                <?php
            	if(count($cat)>0)
            	{
            	?>
                <ul role="menu" class="dropdown-menu">
                    <?php for($j=0,$count_c=count($cat);$j<$count_c;$j++){ 
                        $href_cat = 'gioi-thieu/'.$cat[$j]["tenkhongdau"].'-'.$cat[$j]["id"].'.html';
                    ?>
                    <li id="menu-item-9" class="menu-item menu-item-type-taxonomy menu-item-object-category menu-item-9"><a title="<?=$cat[$j]["ten_en"]?>" href="<?=$href_cat?>"><?=$cat[$j]["ten_en"]?></a></li>
                    <?php }?>
                </ul>
                <?php }?>
            </li>
    ...
    <?php for($i=0,$count_l=count($list);$i<$count_l;$i++){
            $d->reset();
            $sql_cat ="select *  from #_product_cat where id_list='".$list[$i]["id"]."' and ( hienngonngu =3 or hienngonngu = 2) order by stt asc";
            $d->query($sql_cat);
            $cat =$d->result_array();
            $child = 'class="has-submenu" id="sm-15003598033380005-5" aria-haspopup="true" aria-controls="sm-15003598033380005-6" aria-expanded="false"';
            $href = 'danh-muc-list/'.$list[$i]["tenkhongdau"].'-'.$list[$i]["id"].'.html';
    ...
    ?>
    <li id="menu-item-2747" class="<?=$dropdown?>">
        <a title="<?=$list[$i]["ten_en"]?>" href="<?php echo $href;?>" <?php echo $dropdown_toggle;?> ><?=$list[$i]["ten_en"]?><?=$caret?></a>
        <?php
    	if(count($cat)>0)
    	{
    	?>
        <ul role="menu" class=" dropdown-menu">
            <?php for($j=0,$count_c=count($cat);$j<$count_c;$j++){ 
                $d->reset();
                $sql_item ="select *  from #_product_item where id_list='".$list[$i]["id"]."' and  id_cat='".$cat[$j]["id"]."' and ( hienngonngu =3 or hienngonngu = 2) order by stt asc";
                $d->query($sql_item);
                $item =$d->result_array();
                $href_cat = "danh-muc-cat/".$cat[$j]["tenkhongdau"]."-".$cat[$j]["id"].".html";
                $dropdown = "";
            ...
            ?>
            <li id="menu-item-297" class="menu-item menu-item-type-taxonomy menu-item-object-category menu-item-has-children menu-item-297 <?=$dropdown?>">
                <a title="<?=$cat[$j]["ten_en"]?>" href="<?=$href_cat?>"><?=$cat[$j]["ten_en"]?></a>
                <?php
            	if(count($item)>0)
            	{
            	?>
                <ul role="menu" class=" dropdown-menu">
                    <?php for($k=0,$count_i=count($item);$k<$count_i;$k++){ ?>
                        <li id="menu-item-309" class="menu-item menu-item-type-taxonomy menu-item-object-category menu-item-309"><a title="<?=$item[$k]["ten_en"]?>" href="danh-muc-item/<?=$item[$k]["tenkhongdau"]?>-<?=$item[$k]["id"]?>.html"><?=$item[$k]["ten_en"]?></a></li>
                    <?php }?>
                </ul>
                <?php }?>
            </li>
    ...
    for($i=0,$count_l=count($list);$i<$count_l;$i++){
            $d->reset();
            $sql_cat ="select *  from #_tinloai1_1_cat where id_list='".$list[$i]["id"]."' and ( hienngonngu =3 or hienngonngu = 2) order by stt asc";
            $d->query($sql_cat);
            $cat =$d->result_array();
            $child = 'class="has-submenu" id="sm-15003598033380005-5" aria-haspopup="true" aria-controls="sm-15003598033380005-6" aria-expanded="false"';
            $href = 'tin-tuc-list/'.$list[$i]["tenkhongdau"].'-'.$list[$i]["id"].'.html';

...
<?php 
			...
			$sql_tinl="select * from #_tinloai1_1 where hienthi =1 and ( hienngonngu =3 or hienngonngu =1) and id_cat='".$id."' order by id desc";
			$d->query($sql_tinl);	
...
<?php 
			...
			$d->reset();
			$id =  addslashes($_GET['id']);
			$sql_tinl="select * from #_tinloai1_1 where hienthi =1 and ( hienngonngu =3 or hienngonngu =2) and id_cat='".$id."' order by id desc";
			$d->query($sql_tinl);	
			$result_tinl=$d->result_array();		
			$curPage = isset($_GET['p']) ? $_GET['p'] : 1;
			$url=getCurrentPageURL();
                ...
                <div id="pathway" class="clearfix">
                    <div class="container">
                        <div id="breadcrumbs" itemscope="" itemtype="http://schema.org/BreadcrumbList">
                            ...
                            <span class="hidden-xs">→</span>
                            <span itemprop="itemListElement" itemscope="" itemtype="http://schema.org/ListItem">
                                <a href="<?=$href_list?>" itemprop="item"><span itemprop="name" class="hidden-xs"><?=$result_tinll["ten_en"]?></span></a><meta itemprop="position" content="1">
                            </span> 	
                            <span class="hidden-xs">→</span>
                            <span itemprop="itemListElement" itemscope="" itemtype="http://schema.org/ListItem">
                                <a itemprop="item"><span itemprop="name"><?=$result_tinll_name["ten_en"]?></span></a>
                                <meta itemprop="position" content="2">
                            </span>
                        </div><!-- #breadcrumbs -->
                    </div>
